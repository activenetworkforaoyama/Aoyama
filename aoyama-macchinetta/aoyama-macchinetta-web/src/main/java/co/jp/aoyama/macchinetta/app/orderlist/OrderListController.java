...
import co.jp.aoyama.macchinetta.app.common.ZenkakuKatakanaToHankakuKatakana;
import co.jp.aoyama.macchinetta.app.detail.OrderDetailCoHelper;
import co.jp.aoyama.macchinetta.app.message.MessageKeys;
import co.jp.aoyama.macchinetta.app.order.coinfo.CoNonMember;
import co.jp.aoyama.macchinetta.app.order.coinfo.NonMemberDataResBean;
import co.jp.aoyama.macchinetta.app.order.coinfo.NonMemberDataResListBean;
...
public class OrderListController {
	...
	@Value("${nonMember.url:http://202.214.88.88/member/api/v1/nonmemInfo}")
	URI nonMemberUrl;
	...
	private static final String SEARCHMEI = "ｶﾘ"; 
	...
	public String initSearch(SessionStatus sessionStatus, Model model) {
	    OrderListForm orderListForm = new OrderListForm();
	    Map<String,String> shopTelMap = orderListService.findShopTel(sessionContent.getCategory());
	    orderListForm.setShopTelMap(shopTelMap);
	    String authority = sessionContent.getAuthority();
	    if("04".equals(authority)) {
	    	orderListForm.setPrintReadyOk(true);
	    }
	    model.addAttribute(orderListForm);
		model.addAttribute("initFlag", "0");
		String isDnpFlag = "0";
		model.addAttribute("isDnpFlag", isDnpFlag);
	...
	public String returnInitSearch(SessionStatus sessionStatus, Model model ,HttpServletRequest req) {
		model.addAttribute("initFlag", "1");
		String isDnpFlag = (String) req.getSession().getAttribute("isDnpFlag");
		model.addAttribute("isDnpFlag", isDnpFlag);
		return "orderlist/orderList";
	}
     ...
     * @return 注文情報リスト
     */
	@RequestMapping(value = "/FindAllOrderDnpByCondition", method = RequestMethod.GET)
	@ResponseBody
	public List<Order> FindAllOrderDnpByCondition(@Validated OrderListForm orderListForm,
		...
		//検索条件
		//電話番号 + 業態
		String telAndGyotai = orderListForm.getSearchTelNo();
		
		String searchTelNo = "";
		int gyotai = 0;
		
		if(BaseCheckUtil.isNotEmpty(telAndGyotai)) {
			int telAndGyotaiL = telAndGyotai.length();
			if(13 == telAndGyotaiL) {
				searchTelNo = telAndGyotai.substring(0,12).replaceAll("-", "");
				gyotai = Integer.parseInt(telAndGyotai.substring(12));
			}
		}
		
		//姓名（カナ）
		String searchSei = orderListForm.getSearchSei();
		logger.info("DNP注文検索画面でDNP注文情報を検索する。検索前画面入力「姓名（カナ）：" + searchSei + "」");
		if(BaseCheckUtil.isNotEmpty(searchSei)) {
			searchSei = ZenkakuKatakanaToHankakuKatakana.ZenToHanForKKana(searchSei);
		}
		
		logger.info("DNP注文検索画面でDNP注文情報を検索する。変換後画面入力「姓名（カナ）：" + searchSei + "」");
		logger.info("DNP注文検索条件：「Gyotai:" + gyotai + " Tel:" + searchTelNo + " Sei:" +searchSei + " Mei:" +SEARCHMEI + "」");
		
		List<NonMemberDataResListBean> nonMemberInfo = getNonMemberInfo(gyotai,searchTelNo, searchSei, SEARCHMEI);
		
		if(nonMemberInfo == null || nonMemberInfo.isEmpty()) {
			orderListForm.setOrderList(orderList);
			logger.warn("DNP注文検索画面でDNP注文情報を検索する。検索結果が0件でした。");
			//検索結果list
			return orderList;
		}else {
			List<String> memberList = new ArrayList<String>();
			for (NonMemberDataResListBean nonMemberDataResListBean : nonMemberInfo) {
				logger.info("API検索結果：{}", nonMemberDataResListBean.toString());
				//会員番号
				Long kokno = nonMemberDataResListBean.getKokno();
				if(kokno != null) {
					memberList.add(String.valueOf(kokno));
				}
			}
			if(memberList.isEmpty()) {
				...
				logger.info("DNP注文検索画面でDNP注文情報を検索する。検索結果リスト：{}", memberList.toString());
				//検索条件bean
				OrderCondition orderCondition = beanMapper.map(orderListForm,OrderCondition.class);
				orderList = orderListService.fuzzyQueryDnp(orderCondition,memberList);
				orderListForm.setOrderList(orderList);
				//検索結果list
				return orderList;
			}
     ...
     * @return 注文情報リスト
     */
	@RequestMapping(value = "/FindFactoryOrderByCondition", method = RequestMethod.GET)
	@ResponseBody
	public List<FactoryOrder> FindFactoryOrderByCondition(@Validated OrderListForm orderListForm,
            BindingResult result,
            Model model) {
		orderListForm.setAuthority(sessionContent.getAuthority());
		orderListForm.setBelongCode(sessionContent.getBelongCode());
		...
		OrderCondition orderCondition = beanMapper.map(orderListForm,OrderCondition.class);
		//検索結果list
    	List<FactoryOrder> orderList = orderListService.fuzzyFactoryQuery(orderCondition);
    	
    	if(orderList.size() != 0) {
    		List<FactoryOrder> findPrintStatus = orderListService.findPrintStatus(orderList);
    		for(FactoryOrder factoryOrder : orderList) {
    			for(FactoryOrder findPrintStatu : findPrintStatus) {
    				if(orderCondition.getPrintReadyOk() && factoryOrder.getOrderId().equals(findPrintStatu.getOrderId())) {
    					factoryOrder.setPrintStatus(findPrintStatu.getPrintStatus());
    					break;
        			}
    			}
    		}
	...
	public String gotoOrderCoLink(@PathVariable(value ="orderId") String orderId,
		...
		try {
			...
			model.addAttribute("orderFlag", orderFlag);
			String isDnpFlag = "0";
			req.getSession().setAttribute("isDnpFlag", isDnpFlag);
			//本店オーダー 、商品部の場合
			//店舗の場合
			if (order.getMakerFactoryStatus() != null &&
				order.getShopCode() != null && 
     ...
     * @return String　遷移Controller
     */
	@RequestMapping(value = "/gotoDnpOrderCoLink/{orderId}",method =RequestMethod.GET) 
	public String gotoDnpOrderCoLink(@PathVariable(value ="orderId") String orderId,
	                            Model model,HttpServletRequest req) {
		String authority = sessionContent.getAuthority();
		String userId = sessionContent.getUserId();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		...
		try {
			...
			Order order = orderListService.findDnpOrderByPk(orderId);
			String custCd = order.getCustCd();
			String storeBrandCode = order.getStoreBrandCode();
			String gyotaiCd = "1";
			if(storeBrandCode != null && "01".equals(storeBrandCode)) {
	...
	public String goToOrderCoDivert(@PathVariable(value ="orderId") String orderId,
		...
		String orderFlag = "orderDivert";
		model.addAttribute("orderFlag", orderFlag);
		String isDnpDivertFlag = "0";
		model.addAttribute("isDnpDivertFlag", isDnpDivertFlag);
		return "forward:/orderCo/orderCoUpdate";
	}
	
	@RequestMapping(value = "/goToDnpOrderCoDivert/{orderId}",method =RequestMethod.GET)
	public String goToDnpOrderCoDivert(@PathVariable(value ="orderId") String orderId,
	                       Model model ,HttpServletRequest req) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Order order= orderListService.findDnpOrderByPk(orderId);
		Measuring measuring = measuringService.selectByPrimaryKey(orderId);
...
//        out.close();
     ...
     * @return
     */
	@RequestMapping(value = "/dnpOrderDataCSVOut" , method = RequestMethod.GET)
	public String dnpOrderDataCSVOut(@Validated OrderListForm orderListForm,
								  HttpServletRequest request, 
								  HttpServletResponse response,
					              BindingResult result,
					              Model model)throws Exception {
		...
		if(nonMemberInfo == null || nonMemberInfo.isEmpty()) {
			logger.warn("DNP注文検索画面でDNP注文情報をダウンロードする。API検索結果が0件でした。");
			return null;
		}else {
			List<String> memberList = new ArrayList<String>();
			for (NonMemberDataResListBean nonMemberDataResListBean : nonMemberInfo) {
			...
			if(!nonMemberInfo.isEmpty()) {
				//検索結果list
				orderList = orderListService.findDnpOrderByCondition(orderCondition,memberList);
			}else {
				return null;
			}
		}
...
//		}
		...
		String fileNameDefault = "dnpOrderlist_"+dateToday+".csv";
		try {
			response.addHeader("Content-Type", "application/force-download");
			//デフォルトのダウンロードファイル名を設定する
			response.addHeader("Content-Disposition", "attachment; filename=" + new String(fileNameDefault.getBytes("UTF-8"),"ISO8859-1"));
	...
	public List<NonMemberDataResListBean> getNonMemberInfo(int gyotai ,String searchTelNo,String searchSei,String searchMei) {
		List<NonMemberDataResListBean> nonMemberDataResponse = null;
		try {
			CoNonMember coNonMember = new CoNonMember();
			coNonMember.setTelNo(searchTelNo);
			coNonMember.setSei(searchSei);
			coNonMember.setMei(searchMei);
			coNonMember.setGyotai(gyotai);
			NonMemberDataResBean nonMemberData = restTemplate.postForObject(nonMemberUrl,coNonMember,NonMemberDataResBean.class);
			if(nonMemberData != null) {
				nonMemberDataResponse = nonMemberData.getNonMemberDataResponse();
			}else {
				logger.error("DNP注文検索画面でDNP注文情報を検索する。nonMemberDataがNULLです。");
			}
		}catch(Exception e){
			logger.error("DNP注文検索画面でDNP注文情報を検索する。検索中異常が発生しました。");
	...
	@RequestMapping(value = "/mergerPdfFiles", method = RequestMethod.POST)
	@ResponseBody
	public String MergerPdfFiles(HttpServletRequest request,String orderIdString,String factoryCode) {
		int length = orderIdString.length();
		if(length == 0) {
			return "selectfalse";
		}
		String[] orderId = orderIdString.split(",");
		PdfMerger pdfMerger = new PdfMerger();
		ServletContext servletContext = request.getSession().getServletContext();
    	String out = servletContext.getRealPath("") + "pdf";
    	String returnOut = "Merge("+ factoryCode + ").pdf";
		List<String> checkedWhether = new ArrayList<String>();
		File uploadDir = new File(out);
		   if (!uploadDir.exists()) {
		    uploadDir.mkdir();
		}
		
		String pdfOut = uploadDir + File.separator + "Merge("+ factoryCode + ").pdf";
		
		for(int i = 0;i<orderId.length;i++) {
			checkedWhether.add(orderId[i]);
		}
		try {
			pdfMerger.mergePdf(pdfPath,pdfOut,checkedWhether);
		}
		catch (FileNotFoundException e) {
			e.printStackTrace();
			logger.error(e.toString());
			return "filefalse" + e.getMessage();
		}
		catch (Throwable e) {
			e.printStackTrace();
			logger.error(e.toString());
	...
	@RequestMapping(value = "/printEndToUpdate", method = RequestMethod.POST)
	@ResponseBody
	public String PrintEndToUpdate(String orderIdString) {
		int length = orderIdString.length();
		if(length == 0) {
			return "false";
		}
		String[] orderId = orderIdString.split(",");
		List<String> updatePrintList = new ArrayList<String>();
		
		for(int i = 0;i<orderId.length;i++) {
			updatePrintList.add(orderId[i]);
		}
		orderListService.updateDtbPdfManageStatus(updatePrintList);
		return "true";
	}
